...
class PaymentServiceTest {
    ...
    @Test
    void createPayment_savesPaymentAndReturnsLink() {
        UUID orderId = UUID.randomUUID();
        var request = new CreatePaymentRequest(orderId, BigDecimal.valueOf(5000), "ORD-00001", "user@test.com");

        when(paymentRepository.findByOrderId(orderId)).thenReturn(Optional.empty());
        when(tochkaApiClient.createPayment(any())).thenReturn(new TochkaCreateResponse("op-1", "https://pay.link"));
        when(paymentRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        PaymentResponse result = paymentService.createPayment(request);

        assertThat(result.paymentLink()).isEqualTo("https://pay.link");
    ...
    void updateStatusFromTochka_approved_publishesKafkaEvent() {
        ...
        when(tochkaApiClient.getPaymentStatus("op-1"))
                .thenReturn(new TochkaStatusResponse("op-1", "APPROVED", BigDecimal.valueOf(5000)));
        when(paymentRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
